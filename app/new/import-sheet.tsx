...
function buildAiPrompt(ref: CatalogReferenceDto): string {
  const lines: string[] = [
    ...
    "۹) «دسته» و «زیردسته» را از درخت دسته‌بندی‌های موجود در JSON زیر انتخاب کن. اگر کالا در دسته جدیدی قرار می‌گیرد که در درخت نیست، نام دسته و زیردسته مناسب را بنویس — سیستم خودکار آن‌ها را در درخت دسته‌بندی ایجاد می‌کند.",
    "",
...
export function ImportSheet({
  ...
  const downloadTemplate = () => {
    ...
    // مقادیر با " quote می‌شوند تا کامای داخل متن فارسی مشکل نسازد.
    const rows = [
      ["نام کالا", "برند", "بسته‌بندی", "قیمت فروش", "موجودی", "حداقل سفارش", "حجم خرید", "لینک عکس", "دسته", "زیردسته"],
    ...
    // RFC 4180 CSV — مقادیر با " محاصره، " داخل مقدار با "" escape می‌شود
